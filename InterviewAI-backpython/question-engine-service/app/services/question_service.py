...
class QuestionService:
    ...
    Сгенерируй {num_questions} технических вопросов для собеседования. Не больше и не меньше.
    ...
    Сгенерируй строго {num_questions} вопросов. Не больше и не меньше.

    Все вопросы должны быть на русском языке.
    Не используй английский язык, китайский язык, иероглифы или смешанные фразы.
    Не используй символы китайского, японского или корейского письма.
    Не используй испанские, английские или другие нерусские слова, если это не название технологии.
    
    Вопросы должны соответствовать профессии и уровню.
    Если профессия Java backend / Java middle:
    обязательно покрыть Java Core, JVM/GC, collections, concurrency, Spring Boot, REST, JPA/Hibernate, transactions, SQL/testing.
    Не задавай вопросы про JavaScript, JavaFX, Swing, AWT, Multicast, если это явно не указано пользователем.
    
    Не повторяй одну и ту же тему.
    Не задавай одинаковые вопросы другими словами.
    Не сравнивай понятие само с собой.
    Не используй некорректные утверждения в вопросах.
    Не делай все вопросы про одну тему.
